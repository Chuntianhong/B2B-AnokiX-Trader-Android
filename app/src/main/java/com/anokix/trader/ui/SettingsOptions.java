...
public final class SettingsOptions {
    ...
    // Currency — display code shown in the field, value sent to the API.
    public static final String[] CURRENCY_LABELS = {"ZAR", "USD", "EUR"};
    public static final String[] CURRENCY_VALUES = {"1", "2", "3"};
    public static final String[] CURRENCY_CODES  = {"ZAR", "USD", "EUR"};
    ...
    /** Map a stored currency ("ZAR"/"USD"/"EUR" code or "1"/"2"/"3" id) to its option index. */
    public static int currencyIndex(String stored) {
        if (stored == null) return 0;
        for (int i = 0; i < CURRENCY_VALUES.length; i++) {
            if (CURRENCY_VALUES[i].equals(stored) || CURRENCY_CODES[i].equalsIgnoreCase(stored)) return i;
        }
        return 0;
    }
